...
ROLLBACK;
SELECT * FROM DEPT04;
TRUNCATE TABLE DEPT04; -- DDL(DATA DEFINITION LANGUAGE) 방식. 속도 빠르지만 원복 불가
-- 테이블 삭제 명령어: 테이블과 데이터 자체를 삭제
-- DROP TABLE 테이블 명
DROP TABLE EMP04;
SELECT * FROM EMP04; -- 삭제된 테이블은 존재하지 않음..
-- EX1) DEPT05를 삭제 처리..
-- EX2) HERO_LIST01 테이블을 생성, 컬럼추가, 컬럼변경, 컬럼명 변경, 컬럼삭제, TURNCATE로 데이터 삭제, 테이블 삭제
CREATE TABLE HERO_LIST01
AS SELECT * FROM HERO_LIST;
SELECT * FROM HERO_LIST01;
ALTER TABLE HERO_LIST01
ADD POINT NUMBER;
ALTER TABLE HERO_LIST01
...
ALTER TABLE HERO_LIST01
DROP COLUMN POINT; 
TRUNCATE TABLE HERO_LIST01;
DROP TABLE HERO_LIST01;

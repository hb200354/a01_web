...
3. 연산자 4가지
	1) UNION : 합집합 - 두 결과를 합침(중복 제거함)
	2) UNION ALL : 합집합 - 두 결과 합침(중복 제거하지 않음 - 둘다 포함 표현)
	3) INTERSECT : 교집합 - 두 결과에 공통으로 존재하는 행 - 당연히 중복 없음
	4) MINUS : 차집합 - A 결과 - B결과 (당연히 중복 없음)
	
	- 유의 사항
		컬럼명 : 최종 결과의 컬럼 이름은 첫 번째 select문의 컬럼 이름을 따른다.
		정렬(order by) 정렬은 가장 마지막 select 문 뒤에 딱 한번만 사용할 수 있다.
